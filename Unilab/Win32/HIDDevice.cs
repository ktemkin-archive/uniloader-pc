...
namespace Unilab.Win32
{
	...
	/// <summary>
	///  Basic HID Device controller
	/// </summary>
    public class HIDDevice : API.USB, IDisposable
    {
		...
		private void Initialise(string strPath)
		{
			...
			if ( m_hHandle != InvalidHandleValue )	// if the open worked...
			{
				...
				if (HidD_GetPreparsedData(m_hHandle, out lpData))	// get windows to read the device data into an internal buffer
				{
					...
					{
						HidCaps oCaps;
						HidP_GetCaps(lpData, out oCaps);	// extract the device cpabilities from the internal buffer
        ...
        protected void ReadCompleted(IAsyncResult iResult)
        {
            ...
            try
            {
                ...
                try
                {
					HIDInputReport oInRep = CreateInputReport();	// Create the input report for the device
        ...
        public void Write(HIDOutputReport oOutRep)
        {
			try
			{
		...
		protected virtual void HandleDataReceived(HIDInputReport oInRep)
		{
		}
		/// <summary>
		...
		public static HIDDevice FindDevice(int nVid, int nPid)
        {
            ...
            try
            {
				...
				while (SetupDiEnumDeviceInterfaces(hInfoSet, 0, ref gHid, (uint)nIndex, ref oInterface))	// this gets the device interface information for a device at index 'nIndex' in the memory block
                {
                    ...
                    {
                        HIDDevice oNewDevice = new HIDDevice();	// create an instance of the class for this device
		...
		public virtual HIDInputReport CreateInputReport()
		{
			return null;
		}
		#endregion
